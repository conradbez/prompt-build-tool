"""
Concrete model-type handlers for ``loop`` and ``execute_python``.

Each class inherits from ``BaseModelHandler`` and overrides ``execute_node``
with the appropriate execution strategy.  Instances are registered in
``executor.py`` at module-load time.
"""
...
class ExecutePythonModelHandler(BaseModelHandler):
    ...
    async def execute_node(
        ...
        with redirect_stdout(stdout_buf):
            exec(compile(rendered, f"<{model.name}>", "exec"), local_ns)  # noqa: S102
