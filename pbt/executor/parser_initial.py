...
# are declared by the kind itself and come from the registry, so registering a
# kind is the only step needed to make its keys legal.  Anything left over is either a typo or a key consumed by a custom
